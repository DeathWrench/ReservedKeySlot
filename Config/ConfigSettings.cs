...
namespace ReservedKeySlot.Config
{
    public static class ConfigSettings
    {
        //public static ConfigEntry<bool> hideKeyMeshShoulder;
        ...
        public static void BindConfigSettings()
        {
            ...
            //hideKeyMeshShoulder = AddConfigEntry(Plugin.instance.Config.Bind("Client-side", "HideKeyOnShoulder", false, "Hides the Key mesh while on your shoulder. Only applies in scenarios where you can view your player in third person."));
            overrideItemSlotPriority = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "KeySlotPriorityOverride", -40, "[Host only] Manually set the priority for this item slot. Higher priority slots will come first in the reserved item slots, which will appear below the other slots. Negative priority items will appear on the left side of the screen, this is disabled in the core mod's config."));
            overridePurchasePrice = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "KeySlotPriceOverride", 0, "[Host only] Manually set the price for this item in the store. Setting 0 will force this item to be unlocked immediately after the game starts."));
            additionalItemsInSlot = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "AdditionalItemsInSlot", "Lockpicker", "[Host only] Syntax: \"Item1,Item name2\" (without quotes). When adding items, use the item's name as it appears in game. Include spaces if there are spaces in the item name. Adding items that do not exist, or that are from a mod which is not enabled will not cause any problems.\nNOTE: IF YOU ARE USING A TRANSLATION MOD, YOU MAY NEED TO ADD THE TRANSLATED NAME OF ANY ITEM YOU WANT IN THIS SLOT."));
